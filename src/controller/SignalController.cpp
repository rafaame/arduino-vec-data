...
void SignalController::init() {
	...
	pinMode(SIGNAL_CRANKING_PIN, INPUT);
	pinMode(SIGNAL_DIAGNOSIS_PIN, INPUT);
	pinMode(SIGNAL_UC_PIN, INPUT);
	pinMode(SIGNAL_ALTERNATOR_PIN, INPUT);
	pinMode(SIGNAL_IMOBILIZER_PIN, INPUT);
	pinMode(SIGNAL_AIR_CONDITIONING_PIN, INPUT);
	pinMode(SIGNAL_DEFROST_PIN, INPUT);
	pinMode(SIGNAL_AIR_RECIRCULATION_PIN, INPUT);
}
