...
DataHandler::DataHandler(StateController *stateController, const ELM327Controller *elm327Controller, const SignalController *signalController) {
	this->stateController = stateController;
	this->elm327Controller = elm327Controller;
	this->signalController = signalController;

	packetBuffer = new uint8_t[128];
...
void DataHandler::loop() {
	const DataPacket *packet = readPacket();
	packet->toByteArray(packetBuffer);

	delete packet;
}

uint8_t *DataHandler::getPacketBuffer() {
	return packetBuffer;
}
...
void DataHandler::printDataPacket(DataPacket *packet) {
